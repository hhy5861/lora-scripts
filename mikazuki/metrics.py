...
def record_training_start(model_type: str):
    """记录训练开始"""
    flux_training_status.labels(
        model_type=model_type
    ).set(1)  # running

def record_training_running(model_type: str):
    """记录训练进行中"""
    flux_training_status.labels(
        model_type=model_type
    ).set(1)  # running

def record_training_completed(model_type: str):
    """记录训练完成"""
    flux_training_status.labels(
        model_type=model_type
    ).set(2)  # completed

def record_training_failed(model_type: str, error_type: str = 'unknown'):
    """记录训练失败"""
    flux_training_status.labels(
        model_type=model_type
    ).set(3)  # failed

def record_training_progress(model_type: str, progress_type: str, percent: float):
    """记录训练进度"""
...
def record_training_steps(model_type: str, current_steps: int, total_steps: int):
    """记录训练步数"""
...
def record_training_loss(model_type: str, avg_loss: float):
    """记录训练损失"""
    flux_training_loss.labels(
        model_type=model_type
    ).set(avg_loss)

def record_training_iteration_time(model_type: str, iteration_time: float):
    """记录训练迭代时间"""
